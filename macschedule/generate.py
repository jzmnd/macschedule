...
class PListGenerator:
    ...
    def populate_plist(self) -> PListGenerator:
        ...
        if self.config.job.env:
            EnvironmentVariables(
                self.plist_dict, self.config.job.env
            ).generate_elements()
        if self.config.schedule:
            ...
            StartInterval(
                self.plist_dict, self.config.interval.total_seconds()
            ).generate_elements()
        ...
        StandardErrorPath(
            self.plist_dict,
            os.path.expanduser(os.path.join(LOGS_DIR, f"{self.full_job_name}.err")),
        ).generate_elements()
        StandardOutPath(
            self.plist_dict,
            os.path.expanduser(os.path.join(LOGS_DIR, f"{self.full_job_name}.out")),
        ).generate_elements()
        return self
